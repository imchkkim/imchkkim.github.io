...
GW.register("dpo-loss", (el) => {
  ...
  const sTh = GW.slider(f.controls, { label: "<span class=\"sym-pi\">Δ<sub>θ</sub></span>", min: -6, max: 8, step: 0.1, value: st.dth, fmt: (v) => GW.fmt(v, 1), oninput: (v) => { st.dth = v; seg.set(null); draw(); } });
