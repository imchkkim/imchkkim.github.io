...
GW.register("length-bias", (el) => {
  const f = GW.frame(el, {
    ...
    caption:
      ...
      "평균(토큰당 로그확률)으로 비교하면 길이가 사라진다. ORPO는 평균 로그확률을 확률로 되돌린 P̄ 위에서 오즈 log(P̄/(1−P̄))를 비교한다.",
  });
  ...
  add("nw", "y<sub>w</sub> 길이", 1, 40, 1, (v) => v + "토큰");
  add("pw", "y<sub>w</sub> 토큰당 확률", 0.3, 0.99, 0.01, (v) => v.toFixed(2));
  add("nl", "y<sub>l</sub> 길이", 1, 40, 1, (v) => v + "토큰");
  add("pl", "y<sub>l</sub> 토큰당 확률", 0.3, 0.99, 0.01, (v) => v.toFixed(2));
  GW.legend(f.stage, [["c1", "선호 응답 y<sub>w</sub> (정답)"], ["c2", "비선호 응답 y<sub>l</sub> (오답)"]]);
  ...
  function draw() {
    ...
    tbl.innerHTML =
      "<tr><th>비교 방식</th><th>y<sub>w</sub></th><th>y<sub>l</sub></th><th>이기는 쪽</th></tr>" +
      rows.map(([name, a, b]) =>
        `<tr><td>${name}</td><td>${GW.fmt(a, 2)}</td><td>${GW.fmt(b, 2)}</td><td>${a > b ? "✓ 정답 y<sub>w</sub>" : a < b ? "✗ 오답 y<sub>l</sub>" : "무승부"}</td></tr>`
      ).join("");
