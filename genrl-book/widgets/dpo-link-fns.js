...
GW.register("dpo-link-fns", (el) => {
  ...
  GW.legend(f.stage, [
    ["c1", "DPO: −log σ(<span class='sym-beta'>β</span><span class='sym-R'>z</span>)"],
    ["c2", "IPO: (<span class='sym-R'>z</span> − 1/(2<span class='sym-beta'>β</span>))²"],
  ...
  function draw() {
    ...
    f.readout.innerHTML =
      `<span class='sym-R'>z</span> = 6 에서 DPO 손실 <b>${F.dpo(6).toFixed(3)}</b> (0이 아님) · IPO 타깃 <span class='sym-R'>z*</span> = <b>${zI.toFixed(2)}</b> · ` +
      `힌지가 멈추는 <span class='sym-R'>z</span> = <b>${zH.toFixed(2)}</b> · 목표 마진이 DPO 곡선을 민 거리 γ/<span class='sym-beta'>β</span> = <b>${(st.gamma / st.beta).toFixed(2)}</b>`;
  }
  c.hover((z) => `<span class='sym-R'>z</span> = ${GW.fmt(z, 2)}<br>DPO ${GW.fmt(F.dpo(z), 3)}<br>IPO ${GW.fmt(F.ipo(z), 3)}<br>힌지 ${GW.fmt(F.hinge(z), 3)}<br>목표 마진 ${GW.fmt(F.tgt(z), 3)}`);
  draw();
});
